...
with webdriver.Firefox(executable_path='Drivers/geckodriver') as driver:
    ...
    time.sleep(1)
    # Find element using CSS selector by id: input#email--1 by class: input.email_class
    user = driver.find_element_by_css_selector("input[id='email--1']")
    # send the email for this component
    user.send_keys('mecomontes@gmail.com')
    passwd = driver.find_element_by_css_selector('input[name="password"]')
    passwd.send_keys('meco002762')
    # Click on the buttom
    button = driver.find_element_by_css_selector('input[name="submit"]')
    button.click()
    time.sleep(5)
